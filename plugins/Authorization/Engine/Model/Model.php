...
namespace Plugins\Authorization\Engine\Model;

use Plugins\Authorization\Engine\Config;
use Plugins\Authorization\Engine\Interfaces\Config as ConfigInterface;
use Plugins\Authorization\Engine\Constants;
 ...
 *
 * @package HKMCode\Phpshots\Access\Model
...
class Model extends Policy
{
     ...
     */
    private function loadAssertion(ConfigInterface $cfg, string $sec, string $key): bool
    {
    ...
    public function addDef(string $sec, string $key, string $value): bool
    {
        ...
        if ('r' == $sec || 'p' == $sec) {
            ...
            $ast->value = stripInlineComments(escapeDotsInAssertion($ast->value));
        }
     ...
     * @throws CasbinException
     */
    private function loadSection(ConfigInterface $cfg, string $sec): void
    {
        $i = 1;
        for (;;) {
