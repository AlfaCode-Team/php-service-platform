...
namespace Plugins\Authorization\Engine;

use Plugins\Authorization\Engine\Effector\DefaultEffector;
use Plugins\Authorization\Engine\Effector\Effector;
use Plugins\Authorization\Engine\Exceptions\CasbinException;
use Plugins\Authorization\Engine\Exceptions\EvalFunctionException;
use Plugins\Authorization\Engine\Exceptions\InvalidFilePathException;
use Plugins\Authorization\Engine\Interfaces\Logger;
use Plugins\Authorization\Engine\Log\Logger\DefaultLogger;
use Plugins\Authorization\Engine\Model\FunctionMap;
...
use Plugins\Authorization\Engine\Interfaces\Persist\Watcher;
use Plugins\Authorization\Engine\Interfaces\Persist\WatcherEx;
use Plugins\Authorization\Engine\RBAC\ConditionalDomainManager as DefaultConditionalDomainManager;
use Plugins\Authorization\Engine\RBAC\ConditionalRoleManager as DefaultConditionalRoleManager;
use Plugins\Authorization\Engine\RBAC\RoleManager as DefaultRoleManager;
use Plugins\Authorization\Engine\RBAC\DomainManager as DefaultDomainManager;
use Plugins\Authorization\Engine\Interfaces\ConditionalRoleManager;
use Plugins\Authorization\Engine\Interfaces\RoleManager;
use Plugins\Authorization\Engine\Util\BuiltinOperations;
use Closure;
...
class CoreEnforcer
{
    ...
    protected function enforcing(string $matcher, &$explains = [], ...$rvals): bool
    {
        ...
        if ('' === $matcher) {
            ...
            $expString = stripInlineComments(escapeDotsInAssertion($matcher));
        }
        ...
        $hasEval = containsEval($expString);
        ...
        if (0 != $policyLen && str_contains($expString, $pType . '_')) {
            foreach ($this->model['p'][$pType]->policy as $policyIndex => $pvals) {
                ...
                if ($hasEval) {
                    $ruleNames = extractEvalParameters($expString);
                    $replacements = [];
                    $pTokens_flipped = array_flip($pTokens);
                    foreach ($ruleNames as $ruleName) {
                        if (isset($pTokens_flipped[$ruleName])) {
                            $rule = escapeDotsInAssertion($pvals[$pTokens_flipped[$ruleName]]);
                    ...
                    $expWithRule = replaceEvalWithMappings($expString, $replacements);
