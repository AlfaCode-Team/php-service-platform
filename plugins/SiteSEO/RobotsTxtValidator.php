...
class RobotsTxtValidator
{
    ...
    public function isUrlAllow($url, $userAgent = '*')
    {
        ...
        foreach ($orderedDirectives as $directiveRow) {
            if ($directiveRow['directive'] == RobotsTxtEditor::DIRECTIVE_ALLOW) {
                $hasAllowDirectives = true;
                break;
            }
        }
        ...
        $isAllow = true;
        foreach ($orderedDirectives as $directiveRow) {
            if (!in_array($directiveRow['directive'], array(RobotsTxtEditor::DIRECTIVE_ALLOW, RobotsTxtEditor::DIRECTIVE_DISALLOW))) {
                continue;
            }
     ...
     * mysql> SELECT directive,value FROM robots_txt where site_id = ?d and directive IN (RobotsTxtEditor::DIRECTIVE_ALLOW,'disallow) AND user_agent = ? ORDER BY CHAR_LENGTH(value) ASC;
     *
     * @param string $userAgent
     * @return array
     */
    ...
    private function orderDirectives(array $rules)
    {
        ...
        $allowRules = !empty($rules[RobotsTxtEditor::DIRECTIVE_ALLOW]) ? $rules[RobotsTxtEditor::DIRECTIVE_ALLOW] : array();
        $disallowRules = !empty($rules[RobotsTxtEditor::DIRECTIVE_DISALLOW]) ? $rules[RobotsTxtEditor::DIRECTIVE_DISALLOW] : array();

        foreach ($allowRules as $rule) {
            $directives[] = array(
                'directive' => RobotsTxtEditor::DIRECTIVE_ALLOW,
                'rule' => $rule,
        ...
        foreach ($disallowRules as $rule) {
            $directives[] = array(
                'directive' => RobotsTxtEditor::DIRECTIVE_DISALLOW,
                'rule' => $rule,
                'rule_regexp' => $this->prepareRegexpRule($rule),
            );
        }
