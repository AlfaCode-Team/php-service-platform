...
export type SetDataByKeyValuePair<TForm extends Record<any, any>> = <
  K extends FormDataKeys<TForm>
>(
  key: K,
  value: FormDataValues<TForm, K>
) => void;
export type SetDataAction<TForm extends Record<any, any>> =
...
/** `true` resets all fields; an array resets only the named fields. */
type ResetOption<TForm extends FormDataType> = boolean | FormDataKeys<TForm>[];

type FormOptions<TForm extends FormDataType = FormDataType> = Omit<
  VisitOptions,
  "data"
> & {
  /** Reset the form (or the given fields) to their defaults after a successful submit. */
  resetOnSuccess?: ResetOption<TForm>;
  /** Reset the form (or the given fields) to their defaults after a failed submit. */
  resetOnError?: ResetOption<TForm>;
};

export interface PageflowFormProps<TForm extends FormDataType> {
  data: TForm;
  isDirty: boolean;
  ...
  replaceData: (newData: Partial<TForm>) => void;
  ...
  submit: (
    ...args:
      | [Method, string, FormOptions<TForm>?]
      | [{ url: string; method: Method }, FormOptions<TForm>?]
  ) => void;
  get: (url: string, options?: FormOptions<TForm>) => void;
  patch: (url: string, options?: FormOptions<TForm>) => void;
  post: (url: string, options?: FormOptions<TForm>) => void;
  put: (url: string, options?: FormOptions<TForm>) => void;
  delete: (url: string, options?: FormOptions<TForm>) => void;
  cancel: () => void;
}
export default function useForm<TForm extends FormDataType>(
  initialValues?: TForm
): PageflowFormProps<TForm>;
export default function useForm<TForm extends FormDataType>(
  rememberKey: string,
  initialValues?: TForm
): PageflowFormProps<TForm>;
export default function useForm<TForm extends FormDataType>(
  ...
  // Always points at the latest `reset` so `submit` can call it without
  // depending on it (reset is declared further down / TDZ-safe via the ref).
  const resetRef = useRef<(...fields: FormDataKeys<TForm>[]) => void>(() => {});
  const rememberKey =
  ...
  const [errors, setErrors] = rememberKey
    ? useRemember(
        {} as Partial<Record<FormDataKeys<TForm>, string>>,
        `${rememberKey}:errors`
      )
  ...
  const submit = useCallback(
    (...args) => {
      ...
      // Pageflow extras — pulled out so they aren't forwarded to the router as
      // (unknown) visit options. `true` resets every field, an array resets only
      // the named fields, back to their current defaults.
      const { resetOnSuccess, resetOnError, ...visitOptions } = options;

      const applyReset = (which) => {
        if (which === true) {
          resetRef.current();
        } else if (Array.isArray(which) && which.length > 0) {
          resetRef.current(...which);
        }
      };
  ...
  const setDefaultsFunction = useCallback(
    (
      fieldOrFields?: FormDataKeys<TForm> | Partial<TForm>,
  ...
  const replaceData = useCallback(
    (newData: Partial<TForm>) => {
      setData({ ...newData } as TForm); // overwrite the entire form data
    },
    [setData]
  );
  ...
  const setError = useCallback(
    (
      fieldOrFields: FormDataKeys<TForm> | Record<FormDataKeys<TForm>, string>,
      maybeValue?: string
    ) => {
      setErrors((errors) => {
        const newErrors = {
  ...
  const clearErrors = useCallback(
    (...fields) => {
      setErrors((errors) => {
        const newErrors = (
          Object.keys(errors) as Array<FormDataKeys<TForm>>
        ).reduce(
          (carry, field) => ({
            ...carry,
