...
function addEventListeners(delay: number): void {
  document.addEventListener('pageflow:start', (e) => start(e, delay))
  document.addEventListener('pageflow:progress', progress)
}
...
function start(event: GlobalEvent<'start'>, delay: number): void {
  ...
  document.addEventListener('pageflow:finish', (e) => finish(e, timeout), { once: true })
}
...
function finish(event: GlobalEvent<'finish'>, timeout: ReturnType<typeof setTimeout>): void {
