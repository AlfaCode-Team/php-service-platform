...
const Renderer = {
  ...
  isPageflowManagedElement(element: Element): boolean {
    return element.nodeType === Node.ELEMENT_NODE && element.getAttribute('pageflow') !== null
  },

  findMatchingElementIndex(element: Element, elements: Array<Element>): number {
    const key = element.getAttribute('pageflow')
    if (key !== null) {
      return elements.findIndex((element) => element.getAttribute('pageflow') === key)
    }
  ...
  update: debounce(function (elements: Array<string>) {
    ...
    const targetElements = Array.from(document.head.childNodes).filter((element) =>
      this.isPageflowManagedElement(element as Element),
    )
...
export default function createHeadManager(
  ...
  function collect(): Array<string> {
    ...
    const defaults: Record<string, string> = {
      ...(title ? { title: `<title pageflow="">${title}</title>` } : {}),
    }
    ...
    const elements = Object.values(states)
      ...
      .reduce((carry, element) => {
        ...
        const match = element.match(/ pageflow="[^"]+"/)
        if (match) {
