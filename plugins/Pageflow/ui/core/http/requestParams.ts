...
export class RequestParams {
  ...
  public headers(): AxiosRequestConfig['headers'] {
    ...
    if (this.isPartial()) {
      headers['X-Pageflow-Partial-Component'] = currentPage.get().component
    }

    const only = this.params.only.concat(this.params.reset)

    if (only.length > 0) {
      headers['X-Pageflow-Partial-Data'] = only.join(',')
    }

    if (this.params.except.length > 0) {
      headers['X-Pageflow-Partial-Except'] = this.params.except.join(',')
    }

    if (this.params.reset.length > 0) {
      headers['X-Pageflow-Reset'] = this.params.reset.join(',')
    }

    if (this.params.errorBag && this.params.errorBag.length > 0) {
      headers['X-Pageflow-Error-Bag'] = this.params.errorBag
    }
