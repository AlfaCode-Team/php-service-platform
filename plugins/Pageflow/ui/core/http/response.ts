...
import { fireErrorEvent, fireInvalidEvent, firePrefetchedEvent, fireSuccessEvent } from '../eventHandler/events'
...
export class Response {
  ...
  public async process() {
    ...
    if (!this.isPageflowResponse()) {
      return this.handleNonPageflowResponse()
    }
  ...
  protected async handleNonPageflowResponse() {
    if (this.isLocationVisit()) {
      const locationUrl = hrefToUrl(this.getHeader('x-pageflow-location'))
  ...
  protected isPageflowResponse(): boolean {
    return this.hasHeader('x-pageflow');
  }
  ...
  protected isLocationVisit(): boolean {
    return this.hasStatus(409) && this.hasHeader('x-pageflow-location')
  }

  /**
   * Performs an external redirect: navigate the browser to the given URL,
   * preserving scroll state across the full page load when requested.
   */
  ...
  protected async setPage(): Promise<void> {
    ...
    // if(this.requestParams.all().loadPage){
    return currentPage.set(pageResponse, {
      replace: this.requestParams.all().replace,
      loadPage: this.requestParams.all().loadPage,
