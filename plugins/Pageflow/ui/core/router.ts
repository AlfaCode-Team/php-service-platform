...
import { fireBeforeEvent } from './eventHandler/events'
...
import { RequestStream } from './http/requestStream'
...
export class Router {
  ...
  protected loadDeferredProps(): void {
    ...
    // Batch every deferred group into a SINGLE partial reload instead of one
    // request per group — fewer round-trips, same result (props merge in as one
    // response). De-duplicate keys across groups.
    const keys = new Set<string>()
    Object.values(deferred).forEach((group) => {
      ;(group ?? []).forEach((key) => keys.add(key))
    })

    if (keys.size > 0) {
      this.reload({ only: Array.from(keys) })
    }
  }
}
