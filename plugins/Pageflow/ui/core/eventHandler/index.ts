...
import { GlobalEvent, GlobalEventNames, GlobalEventResult, PageflowEvent } from '../types'
import { hrefToUrl } from '../url'

class EventHandler {
  protected pageflowListeners: {
    event: PageflowEvent
  ...
  public onGlobalEvent<TEventName extends GlobalEventNames>(
    ...
    return this.registerListener(`pageflow:${type}`, listener)
  }

  public on(event: PageflowEvent, callback: VoidFunction): VoidFunction {
    this.pageflowListeners.push({ event, listener: callback })

    return () => {
      this.pageflowListeners = this.pageflowListeners.filter((listener) => listener.listener !== callback)
    }
  }
  ...
  public fireInternalEvent(event: PageflowEvent): void {
    this.pageflowListeners.filter((listener) => listener.event === event).forEach((listener) => listener.listener())
  }
