...
`requires: ["tenant.settings", "database.management", "auth.identity", "user.management", "view.rendering"]`
...
`identify(Request): string` returns the tenant id, or `''` for unscoped/central.
...
1. Resolve the active tenant: **encrypted cookie hint first**, then the
   `TenantIdentifier` (see cookie section). `''` → no-op, central DB stays bound.
...
- **User-bound**: stores `{t: tenantId, u: userId}`; ignored unless `u` matches the
  current identity (no cross-user replay). Guests never read/write it.
- **Identifier wins**: a non-empty claim/Host always overrides the cookie, so
  tenant switching and domain routing take effect immediately. The cookie is only
  consulted when the identifier returns `''`.
