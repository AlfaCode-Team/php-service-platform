# AlfacodeTeam PhpServicePlatform — Error Handling Context
...
```php
// ── Domain layer ─────────────────────────────────────────────────────────
// Use built-in \DomainException (not a AlfacodeTeam PhpServicePlatform type)
...
- Branded "Sentinel" (matches the kernel docs / `sentinel/kernel`).
...
When generating or reviewing error handling code:
...
- **DON'T** throw generic `\Exception` or `\RuntimeException` — always a AlfacodeTeam PhpServicePlatform subclass
